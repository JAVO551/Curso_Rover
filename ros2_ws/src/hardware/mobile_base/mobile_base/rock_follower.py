...
class PathPlanner(Node):
    ...
    def target_callback(self, msg):
        ...
        self.last_msg_time = time.time()
        self.get_logger().info(f"Target ROCK received: x={self.target_x:.3f}, z={self.target_z:.3f}")


    def move_callback(self, msg):
        self.move = True
        self.get_logger().info(f"Follow rock command received: {msg.data}")
    ...
    def control_loop(self):
        ...
        if self.state == SM_WAITING:
                ...
                self.publisher_vel.publish(msg)
                self.get_logger().info(f"APPROACHING ROCK: x={self.target_x:.3f}, z={self.target_z:.3f}, v={msg.linear.x:.3f}, w={msg.angular.z:.3f}")
